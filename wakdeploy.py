...
post_targets = [3509, 3751, 3517, 3761, 3523, 3763, 3529, 3769, 3539, 3781, 3547, 3793, 3557, 3803, 3643, 3889, 3653, 3901, 3671, 3917, 3677, 3923, 3709, 3953, 3733, 3973, 3743, 3989, 3749, 3991]
post_var = 0
read_db = {'a':[], 'b':[], 'c':[], 'd':[], 'e':[], 'f':[], 'g':[], 'h':[], 'i':[], 'j':[], 'k':[], 'l':[], 'm':[], 'n':[], 'o':[], 'p':[], 'q':[], 'r':[], 's':[], 't':[], 'u':[], 'v':[], 'w':[], 'x':[], 'y':[], 'z':[], 'chasy':[]}
    ...
    if time.localtime()[3] >= 7: #this section starts the day activities and picks the post target for the day
        ...
        if serum == False:
            ...
            if pager == 0:
                ...
                first_post_box = driver.find_element_by_class_name('blog-list-details') #this section would select the first post ont he landing page and proceed
                la_herd = first_post_box.find_elements_by_class_name('item-details')
                first_post = la_herd[serum_no].find_element_by_tag_name('a')
                first_post.click()
                WebDriverWait(driver, 5).until(EC.visibility_of_element_located((By.CLASS_NAME, 'plan2-recommended')))
                driver.refresh()
        ...
        while completed < todays_post_target:
            ...
            if landage > len(post_herd): #if the landage variable is greater than the number of articles present go to the next page on the pagination
                ...
                toronto = False #this block of code searches for the articles that has not been read by checking them against the read_db dictionary and when a particular one is not in it it appends it to the list and leaves it as the selected one to be opened
                while toronto == False:
                    if new_post_herd[landage].text[0].lower() in read_db.keys():
                        if new_post_herd[landage].text in read_db[new_post_herd[landage].text[0].lower()]:
                            landage += 1
                        else:
                            read_db[new_post_herd[landage].text[0].lower()].append(new_post_herd[landage].text)
                            break
                    else:
                        if new_post_herd[landage].text in read_db['chasy']:
                            landage += 1
                        else:
                            read_db['chasy'].append(new_post_herd[landage].text)
                            break
